...
|---|---|---|---|
| `gpt5-apac` | gpt-5.2 (OpenAI) | 🌏 APAC（越境） | 1（既定・主力。コーディング/ログ解析とも。✅デプロイ済） |
...
| モデル | lifecycle | chatCompletion | DataZone quota（used / limit） | 判定 |
|---|---|---|---|---|
| gpt-5.2 | GA | ✅ true | 50 / 300（空き 250） | ✅ **採用（`gpt5-apac`・主力）** |
| gpt-5.3-codex | GA | ❌ **false**（Responses専用） | 0 / 300（空き 300） | ✗ Zed/VS Code 非対応で不採用 |
| gpt-5.4-mini | GA | ✅ true | **200 / 200（空き 0）** | ✗ quota 枯渇で不採用 |

- `gpt-5.4-mini` の quota は既存 OPSNOTE が消費（prod 150 + eval 50 = 200）。空き 0 のため配備不可
- 当初 `gpt-5.3-codex` を主力にしたが、後述の理由で **`gpt-5.2`（chat 対応・quota 空き 250）に確定**
...
### フェーズ1の実構成（✅ デプロイ済・疎通確認済）

| deployment | モデル | SKU | TPM | 用途 |
|---|---|---|---|---|
| `gpt5-apac` | gpt-5.2 | DataZoneStandard | 200 | 既定・主力（コーディング/ログ解析とも） |

- Chat Completions で疎通確認済み（api-key 認証・IP allowlist・モデル応答 OK）
- 汎用以外に coding 特化が欲しい場合、Responses API クライアント（Codex CLI 等）向けに `gpt-5.3-codex` を別 deployment で追加可
