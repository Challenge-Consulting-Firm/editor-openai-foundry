...
- Zed の `openai_compatible` は既定で `max_tokens` を送るため、
  「Azure Foundry's API returned an unexpected error」になる（[Zed #33103](https://github.com/zed-industries/zed/issues/33103)）
- → **Zed のモデル設定に `"capabilities": {"max_tokens_parameter": false}` を指定**して回避
  （[docs/setup-zed.md](docs/setup-zed.md) の設定例に反映済み）
- あわせて `name` は **deployment 名**（`gpt5-apac`）を指定。モデル ID（`gpt-5.2`）だと 404
