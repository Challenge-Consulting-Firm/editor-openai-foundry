...
### 4. GPT-5 系は旧 `max_tokens` パラメータを拒否（エディタ側設定が必要）

- gpt-5.2 等は `max_tokens` を **400 で拒否**し、`max_completion_tokens` を要求する（実測）。
  エディタ側クライアントは既定で `max_tokens` を送りがちで、クライアントごとに回避設定が異なる:
  - **Zed** (`openai_compatible`): モデル設定に `"capabilities": {"max_tokens_parameter": false}` を指定
    （[Zed #33103](https://github.com/zed-industries/zed/issues/33103)。[docs/setup-zed.md](docs/setup-zed.md) に反映済み）
  - **Continue**: `model` に **"gpt-5" を含むモデル名**（`gpt-5.2`）を書き、deployment 名は `env.deployment` に分離
    （モデル名で変換を判定するため。[continue#7052](https://github.com/continuedev/continue/issues/7052)。[docs/setup-vscode.md](docs/setup-vscode.md) に反映済み）
  - **Copilot BYOK**: 400 が出る場合は `azureModels` の `thinking: true` を設定
- あわせて Zed / Copilot は `name`/モデルID に **deployment 名**（`gpt5-apac`）を指定。モデル ID（`gpt-5.2`）だと 404

### 5. 非OpenAIモデルは実環境では Global のみ（フェーズ2は保留中）

- 公式提供表では DeepSeek-V4-Pro / grok-4.3 / Mistral 系が DataZone(APAC) 提供とされているが、
  **この検証サブスクリプションの list-models では非OpenAIモデルは全て GlobalStandard のみ**（DataZone 無し。2026-07 実測）
- Global = 処理が**米国を含む全世界**に及び得るため、本基盤の越境ポリシー（Global 原則不使用）と両立しない
- → **フェーズ2（DeepSeek 等の追加）は、DataZone で非OpenAIモデルが実提供されるまで保留**。
  現状は `gpt5-apac`（gpt-5.2）単一で運用
- 補足: 候補は全て chat 対応だが、DeepSeek-V4-Pro / Kimi-K2.7-Code / grok-4.3 は **Preview**（GA でない）、
  capacity 上限も小さい（maxCapacity=3）。再開時はこれらも含めて再確認すること
